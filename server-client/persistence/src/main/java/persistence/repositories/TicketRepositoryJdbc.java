...
public class TicketRepositoryJdbc implements ITicketRepository {
    ...
    public TicketRepositoryJdbc(JdbcUtils jdbc) {
    ...
    public List<Ticket> findByShowId(Long showId) {
        List<Ticket> tickets = new ArrayList<>();
        String query = "SELECT * FROM tickets WHERE showId = ?";
        try (Connection con = jdbc.getConnection();
             PreparedStatement stmt = con.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    Ticket ticket = new Ticket(name, seats, showId);
                    ticket.setId(id);
                    tickets.add(ticket);
                }
            }
        } catch (SQLException e) {
            logger.error("Error in findByShowId: {}", e.getMessage());
        }
        return tickets;
    }
}
